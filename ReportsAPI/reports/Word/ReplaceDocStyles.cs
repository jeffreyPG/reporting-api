...
namespace reports.Word
{
    public class ReplaceDocStyles
    {
        ...
        public static void ReplaceOtherStyles(string fromDocument, string toDocument, HtmlData options)
        {
            ...
            try
            {
                ...
                // if the UI has the header, ignore this
                var headerOnUi = options != null && options.HeaderContent != null && 
                                (string.IsNullOrEmpty(options.HeaderContent.Position) == false ||
                                 string.IsNullOrEmpty(options.HeaderContent.Content) == false ||
                                 string.IsNullOrEmpty(options.HeaderContent.Image) == false
                                );

                if (headerOnUi)
                {
                    // dont apply any styles
                }
                else
                {

                    //this code is to change Header Style
                    foreach (MSInterop.Section source in styleDocucument.Sections)
                    {
                        foreach (MSInterop.Section destination in destinationDocument.Sections)
                        {

                            var isHeaderAndIsNotNull = source.Headers.Count > 0 && source.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary] != null && source.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range != null;

                            if (isHeaderAndIsNotNull)
                            {

                                var headerHasFontAndText = source.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.Font != null &&
                                                            !IsEmpty(source.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.Text);

                                if (headerHasFontAndText)
                                {
                                    destination.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.Font = source.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.Font;
                                }

                                style = source.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.get_Style() as MSInterop.Style;
                                if (style != null)
                                {
                                    destination.Headers[MSInterop.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.set_Style(style);
                                }
                            }
                        }
                    }
                ...
                var footerOnUi = options != null && options.FooterContent != null &&
                                (string.IsNullOrEmpty(options.FooterContent.Position) == false ||
                                 string.IsNullOrEmpty(options.FooterContent.Content) == false ||
                                 string.IsNullOrEmpty(options.FooterContent.Image) == false
                                );

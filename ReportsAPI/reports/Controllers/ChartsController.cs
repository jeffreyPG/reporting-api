    ...
    public class ChartsController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> GetViews()
        {
            try
            {
                ...
                string url = baseUrl + "/api/3.6/sites/0d8956fe-3e21-470c-a195-2da4b0ece5e4/views/{0}/image?resolution=high&maxAge=10";
        ...
        public async Task<HttpResponseMessage> GetChart(string viewId, string buildingName, string year = "", string utilType = "")
        {
            try
            {
                //Authenticate the API
                string token = await this.GetAuthToken();
                string url = baseUrl + "/api/3.6/sites/0d8956fe-3e21-470c-a195-2da4b0ece5e4/views/{0}/image?resolution=high&maxAge=10&vf_BuildingId={1}"; //&vf_Year={2}&vf_Utility_Type={3}";
                ...
                var formattedUrl = string.Format(url, viewId, buildingName);

using System;
using System.Linq.Expressions;
using System.Text.RegularExpressions;
using ReportsAPISite.Extensions;

namespace ReportsAPISite.Services.Validators.Validations
{
    public static class IsEmailAddressValidation
    {
        public static IValidator<T> IsEmailAddress<T, TProperty>(this IValidator<T> validator, Expression<Func<T, TProperty>> property)
        {
            var candidate = property.GetCandidateProperties(validator.Candidate);

            const string pattern = @"(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|""(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*"")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])";

            var stringValue = candidate.Value as string ?? string.Empty;
            var isValidEmailAddress = Regex.IsMatch(stringValue, pattern) || string.IsNullOrEmpty(stringValue);

            if (!isValidEmailAddress)
            {
                validator.AddError($"'{stringValue}' is not a valid email address.", candidate.Name);
            }

            return validator;
        }
    }
}